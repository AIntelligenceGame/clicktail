...
func Run(options globals.GlobalOptions) {
	...
	tc := tail.Config{
		...
		Options: tail.TailOptions{
			...
			Stop:      options.Tail.Stop,
			Poll:      options.Tail.Poll,
			StateFile: options.Tail.StateFile,
		},
	}
...
func getParserAndOptions(options globals.GlobalOptions) (parsers.Parser, interface{}) {
...
// sampling, if enabled
func modifyEventContents(toBeSent chan event.Event, options globals.GlobalOptions) chan event.Event {
				...
				for ev := range toBeSent {
					...
					for _, field := range options.ScrubFields {
						if val, ok := ev.Data[field]; ok {
							...
							ev.Data["query_print_sha256"] = fmt.Sprintf("%x", newVal)
						}
					}
					// do adding
...
func makeDynsampleKey(ev *event.Event, options globals.GlobalOptions) string {
